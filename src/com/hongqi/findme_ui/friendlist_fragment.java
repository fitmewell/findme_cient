...
public class friendlist_fragment extends Fragment {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		SimpleAdapter adapter = new SimpleAdapter(getActivity(),
				new getFriendList().getFriendList(HttpUtil.getFriendList(uid)),
				R.layout.friends, new String[] { "uid", "name", "online" },
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				String x = new getFriendList()
						.getFriendLocal(uid_.getText().toString()).get("x")
						.toString();
				String y = new getFriendList()
						.getFriendLocal(uid_.getText().toString()).get("y")
						.toString();
				if (!online.getText().toString().equals("离线")) {
					showTheWay.translateInfo(username, name_.getText()
							.toString(), uid_.getText().toString(), x, y);
				} else {
					Toast toast = Toast.makeText(getActivity(), "好友不在线",
							Toast.LENGTH_SHORT);
					toast.show();
				}
		...
		button.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				getFragmentManager().beginTransaction().addToBackStack(null)
						.add(R.id.pop_menu, new menu_fragment()).commit();
			}
		});
	}
}
